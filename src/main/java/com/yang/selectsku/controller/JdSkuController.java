...
public class JdSkuController
{
    ...
    public String itemAdd2() { return "item-add2"; }


    @RequestMapping({"/JdList2"})
    public String JdList2(Model model) {
        List<Jditem> list = new ArrayList<>();
        list = this.jditemRepository.findByUserId(2);
        model.addAttribute("jdList", list);
        return "JdList2";
    }
    ...
    public String itemAdd3() { return "item-add3"; }


    @RequestMapping({"/JdList3"})
    public String JdList3(Model model) {
        List<Jditem> list = new ArrayList<>();
        list = this.jditemRepository.findByUserId(3);
        model.addAttribute("jdList", list);
        return "JdList3";
    }
    ...
    public String itemAdd4() { return "item-add4"; }


    @RequestMapping({"/JdList4"})
    public String JdList4(Model model) {
        List<Jditem> list = new ArrayList<>();
        list = this.jditemRepository.findByUserId(4);
        model.addAttribute("jdList", list);
        return "JdList4";
    }
    ...
    public String addJdItem(String itemId, String itemName, int time, int byUser) {
        ...
        Jditem result = (Jditem)this.jditemRepository.save(jditem);
        if (byUser == 1) {
    ...
    public String removeJdItem(Long itemId) {
        ...
        this.jditemRepository.deleteById(itemId);
        return "success";
    }
    ...
    public String modifyStatusJdItem(Long id, int status, int byUser) {
        this.jditemRepository.modifyStatus(id, status);
        if (status == 1) {
            Optional<Jditem> optional = this.jditemRepository.findById(id);
    ...
    public String startMonitor(int byUser) {
        List<Jditem> list = this.jditemRepository.findByIdAndStatus(byUser);
        ThreadGroup currentGroup = Thread.currentThread().getThreadGroup();
    ...
    public String stopMonitor(int byUser) {
        ...
        for (int i = 0; i < list.size(); i++) {
            for (int j = 0; j < noThreads; j++) {
                if (lstThreads[j].getName().equals("jd" + ((Jditem)list.get(i)).getId())) {
                    lstThreads[j].stop();
                }
            }
        }

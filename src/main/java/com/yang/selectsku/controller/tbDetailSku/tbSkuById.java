...
public class tbSkuById implements Runnable {
    ...
    public  tbSkuById(String itemId,String skuId,int time,int send ,String itemName,boolean addStart){
        this.itemId=itemId;
        this.skuId=skuId;
        this.time=time;
        this.send=send;
    ...
    public void run() {
        //判断发送的对象 1为白夜 2为群霸
        String sendIds="      \"UID_yV8nb3gdc7I6eYSBRWY0IQP3bcgk\",  \"UID_N5AytME3daIlngtVm6Yt71xx7nrA\", \"UID_EXA4w2hi8PSinrndA9dK4ux8y5yw\" ";
        if(send==1){
...
//        System.out.println(jsonArray3);
            JSONObject jsonArray4 = new JSONObject(jsonArray3.get(skuId) + "");
                    ...
                    if(addStart) {
                        String message =
                                " { \"appToken\":\"AT_Q45yzpNW3dKPNaFF0SLXHZCfMjMcPFrJ\"," +
                                        ...
                                        "  \"summary\":\"开启【淘宝-"+areaName+"】商品详细规格监控库存提醒 " + itemName + " 现在数量为 " + newNum + "<br>"+df.format(new Date())+" \"," +
                                        "  \"topicIds\":[ \n" +
                                        "      1205\n" +
                                        "  ]," +
                                        "  \"contentType\":2, " +
                ...
                if(firstCome) {
                    if(addStart) {
                        String message =
                                " { \"appToken\":\"AT_Q45yzpNW3dKPNaFF0SLXHZCfMjMcPFrJ\"," +
                                        "  \"content\":\" " + title + "库存为" + newNum + "\"," +
                                        "  \"summary\":\"开启【淘宝-"+areaName+"】商品详细规格监控库存 " + itemName + " 现在数量为 " + newNum + "<br>"+df.format(new Date())+" \"," +
                                        "  \"topicIds\":[ \n" +
                                        "      1205\n" +
                                        "  ]," +
                                        "  \"contentType\":2, " +
            ...
            if(errorSend){
                if(e.getMessage().contains("JSONObject[\"item\"] not found")){
                    String message =
                            " { \"appToken\":\"AT_Q45yzpNW3dKPNaFF0SLXHZCfMjMcPFrJ\"," +
                                    "  \"content\":\" 商品id设置错误  \"," +
                                    "  \"summary\":\"【淘宝-"+areaName+"】商品 " + itemName + " id或者skuid设置错误，请检查 <br>"+df.format(new Date())+" \"," +
                                    "  \"topicIds\":[ \n" +
                                    "      1205\n" +
                                    "  ]," +
                                    "  \"contentType\":2, " +
